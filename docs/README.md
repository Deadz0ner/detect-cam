...
Short notes on how each piece of the project works. Read them in order —
each one builds on the previous.

| # | File | What it covers |
|---|---|---|
| 01 | [flow.md](01-flow.md) | The end-to-end pipeline: setup → loop → cleanup, and the project file layout |
...
| 05 | [detection-check.md](05-detection-check.md) | Deciding if a person is inside the ROI (`feet` vs `bbox`) |
...
| 10 | [aabb-fastreject.md](10-aabb-fastreject.md) | The two-step (AABB + mask) overlap check used in `bbox` mode |
